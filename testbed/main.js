...
function createSystem(config) {
  ...
  const origSubscribe = system2.subscribe.bind(system2);
  system2.subscribe = function subscribeProxy(evt, handler) {
...
function bindComponent(host, root) {
  ...
  for (const el of elements) {
    ...
    switch (binding.type) {
      case "text": {
        el.textContent = String(system2.readField(binding.field) ?? "");
        const unsub = system2.onFieldChange(binding.field, (val) => {
      ...
      case "bind": {
        ...
        binding.fields.forEach((fieldName, i) => {
          vals[i] = system2.readField(fieldName);
        });
        applyBindTemplate(el, binding, vals);
        binding.fields.forEach((fieldName, i) => {
          const unsub = system2.onFieldChange(fieldName, (val) => {
            vals[i] = val;
            applyBindTemplate(el, binding, vals);
          });
          unbindFns.push(unsub);
      ...
      case "toggle": {
        const htEl = el;
        htEl.hidden = !Boolean(system2.readField(binding.field));
        const unsub = system2.onFieldChange(binding.field, (val) => {
      ...
      case "on": {
        const evtObj = system2.event(binding.targetEvent);
        if (!evtObj) {
...
// testbed/main.ts
var hp = field("hp", { default: 20 });
var hpMax = field("hpMax", { default: 20 });
var tempHp = field("tempHp", { default: 0 });
var status = field("status", { default: "healthy" });
var bloodied = field("bloodied", { default: false });
var hpPercent = field("hpPercent", { default: 100 });
var damageTaken = event("damage:taken");
var healApplied = event("heal:applied");
var tempGained = event("temp:gained");
...
useSystem(system);
var systemEl = document.getElementById("btn-damage-5");
systemEl.onclick = () => {
...
var hpTemplate = document.getElementById("tpl-hp-bar");
var statusTemplate = document.getElementById("tpl-status-badge");
